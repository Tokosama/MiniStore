...
export default function ProductPage({ backet, setBacket, addToBacket }) {
  ...
  const [productStock, setProductStock] = useState(product[0].stock);
  const [showSkeleton, setShowSkeleton] = useState(false);

  useEffect(() => {
    function calculProductStock() {
      const isProductInBacket = backet.filter(
        (item) => item.name == product[0].name,
      );

      if (isProductInBacket.length) {
        setProductStock(product[0].stock - isProductInBacket[0]?.quantity);
      }
    }
    setTimeout(() => {
      calculProductStock();
          ...
          <div className=" py-8 mx-auto max-w-md sm:max-w-7xl px-2 sm:px-6 lg:px-8">
            <div className="grid md:grid-cols-2 sm:gap-4 mt-4">
              ...
              <div className=" py-8  relative ">
                ...
                {product[0].stock == 0 && (
                  <div className=" py-6 mb-auto text-[#E85A64]">
                    stocke épuisé
                  </div>
                )}
                {product[0].stock && (
                ...
                <div className="grid md:grid-cols-2  bottom-5  ">
                  <div className="mb-5">
                    <div className="text-gray-500 text-xs mb-[-3px] ">
                      PRICE
                    </div>
